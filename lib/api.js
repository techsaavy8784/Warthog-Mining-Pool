...
function collectStats() {
  ...
  var windowTime = (
    (Date.now() / 1000 - config.api.hashrateWindow) |
    0
  ).toString();
  ...
  async.parallel(
    {
      pool: function (callback) {
        redisClient.multi(redisCommands).exec(function (error, replies) {
          ...
          var data = {
            ...
            totalMinersPaid:
              replies[10] && replies[10].length > 0
                ? replies[10].length - 1
                : 0,
            miners: 0,
            workers: 0,
          ...
          for (var i = 0; i < hashrates.length; i++) {
            ...
            minersHashrate[hashParts[1]] =
              (minersHashrate[hashParts[1]] || 0) + parseInt(hashParts[0]);
          }
          ...
          for (var miner in minersHashrate) {
            ...
            minersHashrate[miner] = Math.round(
              minersHashrate[miner] / config.api.hashrateWindow
            );
      ...
      config: function (callback) {
        callback(null, {
          ...
          blocksChartEnabled:
            config.charts.blocks && config.charts.blocks.enabled,
          blocksChartDays:
            config.charts.blocks && config.charts.blocks.days
              ? config.charts.blocks.days
              : null,
          telegramBotName:
            config.telegram && config.telegram.botName
              ? config.telegram.botName
              : null,
          telegramBotStats:
            config.telegram && config.telegram.botCommands
              ? config.telegram.botCommands.stats
              : "/stats",
          telegramBotReport:
            config.telegram && config.telegram.botCommands
              ? config.telegram.botCommands.report
              : "/report",
          telegramBotNotify:
            config.telegram && config.telegram.botCommands
              ? config.telegram.botCommands.notify
              : "/notify",
          telegramBotBlocks:
            config.telegram && config.telegram.botCommands
              ? config.telegram.botCommands.blocks
              : "/blocks",
        });
      },
      charts: function (callback) {
        ...
        charts.getPoolChartsData(function (error, data) {
          ...
          // Blocks chart
          if (
            !config.charts.blocks ||
            !config.charts.blocks.enabled ||
            !config.charts.blocks.days
          ) {
            callback(error, data);
            return;
          ...
          if (chartDays === 1) {
            ...
            for (var d = 0; d <= chartDays; d++) {
              var date = utils.dateFormat(
                new Date((beginAtTimestamp + d * 86400) * 1000),
                "yyyy-mm-dd"
              );
              blocksCount[date] = 0;
            }
    ...
    function (error, results) {
      log(
        "info",
        logSystem,
        "Stat collection finished: %d ms redis, %d ms daemon",
        [redisFinished - startTime, daemonFinished - startTime]
      );
...
function getNetworkData(callback) {
  apiInterfaces.getChainHead(function (error, reply) {
    if (error) {
      log("error", logSystem, "Error getting network data %j", [error]);
      callback(true);
      return;
    } else {
      var blockHeader = reply;
      callback(null, {
        difficulty: blockHeader.difficulty,
        height: blockHeader.pinHeight,
      });
    }
  });
}
...
function broadcastWorkerStats(address, destinations) {
  ...
  redisClient.multi(redisCommands).exec(function (error, replies) {
    ...
    var stats = replies[0];
    stats.hashrate =
      minerStats[address] && minerStats[address]["hashrate"]
        ? minerStats[address]["hashrate"]
        : 0;
    stats.roundHashes =
      minerStats[address] && minerStats[address]["roundHashes"]
        ? minerStats[address]["roundHashes"]
        : 0;
    if (replies[3]) {
    ...
    for (var j = 0; j < replies[2].length; j++) {
      ...
      if (miner.indexOf("~") !== -1) {
        ...
        var workerData = {
          name: workerName,
          hashrate:
            minerStats[miner] && minerStats[miner]["hashrate"]
              ? minerStats[miner]["hashrate"]
              : 0,
        };
          ...
          for (var i in workersData) {
            ...
            if (replies[wi]) {
              workersData[i].lastShare = replies[wi]["lastShare"]
                ? parseInt(replies[wi]["lastShare"])
                : 0;
              workersData[i].hashes = replies[wi]["hashes"]
                ? parseInt(replies[wi]["hashes"])
                : 0;
            }
            if (replies[hi]) {
...
function handleTopMiners(response) {
  async.waterfall(
        ...
        redisClient.multi(redisCommands).exec(function (error, redisData) {
          ...
          for (var i in redisData) {
            ...
            minersData.push({
              ...
              hashrate:
                minersHashrate[address] && minerStats[address]["hashrate"]
                  ? minersHashrate[address]
                  : 0,
              lastShare: data[0],
              hashes: data[1],
            });
    ...
    function (error, data) {
      if (error) {
        response.end(
          JSON.stringify({ error: "Error collecting top 10 miners stats" })
        );
        return;
      }
    ...
    function (error, value) {
      if (error) {
        response.end(
          JSON.stringify({
            status:
              "Unable to get the current minimum payout level from database",
          })
        );
        return;
      }
      ...
      var maxLevel = config.payments.maxPayment
        ? config.payments.maxPayment / config.coinUnits
        : null;
...
function handleSetMinerPayoutLevel(urlParts, response) {
  ...
  if (ip.indexOf("*") !== -1 || address.indexOf("*") !== -1) {
    response.end(
      JSON.stringify({ status: "Remove the wildcard from your miner address" })
    );
    return;
  }
  ...
  if (level < minLevel) {
    response.end(
      JSON.stringify({ status: "The minimum payout level is " + minLevel })
    );
    return;
  }
  ...
  if (maxLevel && level > maxLevel) {
    response.end(
      JSON.stringify({ status: "The maximum payout level is " + maxLevel })
    );
    return;
  }
  ...
  minerSeenWithIPForAddress(address, ip, function (error, found) {
    if (!found || error) {
      response.end(
        JSON.stringify({ status: "We haven't seen that IP for your address" })
      );
      return;
    }
      ...
      function (error, value) {
        if (error) {
          response.end(
            JSON.stringify({
              status:
                "An error occurred when updating the value in our database",
            })
          );
          return;
        }
        ...
        log(
          "info",
          logSystem,
          "Updated minimum payout level for " + address + " to: " + payoutLevel
        );
...
function handleSetMinerNotifications(urlParts, response) {
  ...
  if (ip.indexOf("*") !== -1 || address.indexOf("*") !== -1) {
    response.end(
      JSON.stringify({ status: "Remove the wildcard from your input" })
    );
    return;
  }
      ...
      redisClient.hdel(
        ...
        function (error, value) {
          ...
          log(
            "info",
            logSystem,
            "Disabled email notifications for address: " + address
          );
        }
      );
      response.end(JSON.stringify({ status: "done" }));
      ...
      function (error, value) {
        if (error) {
          response.end(
            JSON.stringify({
              status:
                "Unable to get current telegram default miner address from database",
            })
          );
          return;
        }
      ...
      redisClient.hset(
        ...
        function (error, value) {
          ...
          log(
            "info",
            logSystem,
            "Enabled telegram notifications for blocks to " + chatId
          );
        }
      );
      response.end(JSON.stringify({ status: "done" }));
      ...
      redisClient.hdel(
        ...
        function (error, value) {
          ...
          log(
            "info",
            logSystem,
            "Disabled telegram notifications for blocks to " + chatId
          );
        }
      );
      response.end(JSON.stringify({ status: "done" }));
  ...
  if (type === "miner") {
    ...
    redisClient.exists(
      ...
      function (error, result) {
        if (!result) {
          response.end(
            JSON.stringify({ status: "Miner not found in database" })
          );
          return;
        }
          ...
          redisClient.hset(
            ...
            function (error, value) {
              ...
              log(
                "info",
                logSystem,
                "Enabled telegram notifications to " +
                  chatId +
                  " for address: " +
                  address
              );
          ...
          redisClient.hdel(
            ...
            function (error, value) {
              ...
              log(
                "info",
                logSystem,
                "Disabled telegram notifications for address: " + address
              );
            }
          );
          response.end(JSON.stringify({ status: "done" }));
...
function authorize(request, response) {
  ...
  if (
    typeof sentPass == "undefined" &&
    (remoteAddress === "127.0.0.1" ||
      remoteAddress === "::ffff:127.0.0.1" ||
      remoteAddress === "::1" ||
      (bindIp != "0.0.0.0" && remoteAddress === bindIp))
  ) {
    return true;
  }
  ...
  response.setHeader(
    "Set-Cookie",
    "sid=" + authSid + "; path=/; expires=" + cookieExpire.toUTCString()
  );
          ...
          .exec(function (error, replies) {
            if (error) {
              log(
                "error",
                logSystem,
                "Error trying to get admin data from redis %j",
                [error]
              );
              callback(true);
              return;
...
//   };
//   var daemonType = config.daemonType
//     ? config.daemonType.toLowerCase()
//     : "default";
...
function minerSeenWithIPForAddress(address, ip, callback) {
  ...
  redisClient.sismember(
    [config.coin + ":workers_ip:" + address, ip],
    function (error, result) {
...
server.listen(config.api.port, bindIp, function () {
  log("info", logSystem, "API started & listening on %s port %d", [
    bindIp,
    config.api.port,
  ]);
});
...
if (config.api.ssl) {
  if (!config.api.sslCert) {
    log(
      "error",
      logSystem,
      "Could not start API listening on %s port %d (SSL): SSL certificate not configured",
    ...
    log(
      "error",
      logSystem,
      "Could not start API listening on %s port %d (SSL): SSL key not configured",
      [bindIp, config.api.sslPort]
    );
    ...
    ssl_server.listen(config.api.sslPort, bindIp, function () {
      log("info", logSystem, "API started & listening on %s port %d (SSL)", [
        bindIp,
        config.api.sslPort,
      ]);
    });
