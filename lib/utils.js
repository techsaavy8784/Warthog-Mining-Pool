...
var decodedAddress = cnUtil.address_decode(Buffer.from(config.poolServer.poolAddress));
if (decodedAddress === undefined) {
    throw new Error("Failed to decode pool address. Please verify that the pool address is valid.");
}

var addressBase58Prefix = parseInt(decodedAddress.toString(), 10);
var integratedAddressBase58Prefix = config.poolServer.intAddressPrefix ? parseInt(config.poolServer.intAddressPrefix, 10) : addressBase58Prefix + 1;
