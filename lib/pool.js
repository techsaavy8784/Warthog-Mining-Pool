...
function BlockTemplate(template) {
  ...
  this.buffer = Bufer.from(this.header, "hex");
  instanceId.copy(this.buffer, this.reserveOffset + 4, 0, 3);
  this.extraNonce = 0;

  //this.previous_hash = new Buffer(32);
  //this.buffer.copy(this.previous_hash,0,7,39);
  this.writeExtraNonce = function () {
    // Increment extra nonce
    this.extraNonce++;
    // Write the extra nonce at the reserve offset
    this.buffer.writeUInt32BE(this.extraNonce, this.reserveOffset);
  };
}
BlockTemplate.prototype = {
  nextBlob: function () {
    this.writeExtraNonce();
    // Adapt conversion method to Warthog's Janushash algorithm
    return utils.warthogUtil.convert_blob(this.buffer).toString("hex");
  },
};
...
function getBlockTemplate(callback) {
  apiInterfaces.fetchBlockData(callback)
}
  ...
  switch (method) {
    case "login":
      ...
      if (config.poolServer.fixedDiff.enabled) {
        ...
        if (
          fixedDiffCharPos !== -1 &&
          /\D/.test(login.substr(fixedDiffCharPos + 1))
        ) {
          // non-numeric ".whatever": assume it's a worker name
          workerName = login.substr(fixedDiffCharPos + 1);
          login = login.substr(0, fixedDiffCharPos);

          fixedDiffCharPos = login.lastIndexOf(
            config.poolServer.fixedDiff.addressSeparator
          );
        }
...
function newConnectedWorker(miner) {
  log("info", logSystem, "Miner connected %s@%s on port", [
    miner.login,
    miner.ip,
    miner.port,
  ]);
    ...
    function (error, connectedWorkers) {
      if (connectedWorkers === 1) {
        notifications.sendToMiner(miner.login, "workerConnected", {
          LOGIN: miner.login,
          MINER:
            miner.login.substring(0, 7) +
            "..." +
            miner.login.substring(miner.login.length - 7),
    ...
    function (error, connectedWorkers) {
      if (reason === "banned") {
        ...
        notifications.sendToMiner(miner.login, "workerTimeout", {
          ...
          PORT: miner.port,
          WORKER_NAME: miner.workerName !== "undefined" ? miner.workerName : "",
          LAST_HASH: utils.dateFormat(
            new Date(miner.lastBeat),
...
function recordShareData(
  ...
  if (slushMiningEnabled) {
    if (
      lastChecked + config.poolServer.slushMining.lastBlockCheckRate <=
        dateNowSeconds ||
      lastChecked === 0
    ) {
      redisClient.hget(
    ...
    job.score =
      ...
      Math.pow(
        Math.E,
        (dateNowSeconds - scoreTime) / config.poolServer.slushMining.weight
      ); // Score Calculation
    log(
      "info",
      logSystem,
  ...
  var redisCommands = [
    ...
    [
      "expire",
      config.coin + ":payments:" + miner.login,
      86400 * cleanupInterval,
    ],
  ];
  ...
  if (blockCandidate) {
    ...
    redisCommands.push([
      "rename",
      config.coin + ":shares:roundCurrent",
      config.coin + ":shares:round" + job.height,
    ]);
...
function processShare(miner, job, blockTemplate, nonce, resultHash) {
  var template = Buffer.from(blockTemplate.buffer.length);
  ...
  if (
    ...
    log(
      "info",
      logSystem,
      "Mining pool algorithm: %s variant %d, Hard fork version: %d",
      [cnAlgorithm, cnVariant, hard_fork_version]
    );
    shareType = "valid";
  }
      ...
      function (error, result) {
        if (error) {
          ...
          var blockFastHash = utils.cnUtil
            .get_block_id(shareBuffer, cnBlobType)
            .toString("hex");
          log(
            "info",
            logSystem,
