...
module.exports = function(daemonConfig, walletConfig, poolApiConfig){
    return {
        ...
        fetchBlockData: function(callback){
            var blockData = JSON.parse(request('GET', baseurl + `/chain/mine/${config.poolServer.poolAddress}`).body.toString())
            console.log('blockData', blockData)
            return blockData.data
        },
        getChainHead: function(callback) {
            var chainHead = JSON.parse(request('GET', baseurl + '/chain/head').body.toString())
            console.log("chainHead", chainHead)
            return chainHead.data
        },
        submitBlock: function(callback) {
